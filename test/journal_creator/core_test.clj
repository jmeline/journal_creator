...
(deftest verify-utility-functions
  (testing "test get-journal-header-str-str return correct headers"
    (is (= (get-journal-header-str (t/date-time 2017 03 18))
           "= March 18, 2017, Saturday - <SUMMARY GOES HERE> =" ))
    (is (= (get-journal-header-str (t/date-time 2017 03))
           "= March 01, 2017, Wednesday - <SUMMARY GOES HERE> =" ))
    (is (= (get-journal-header-str (t/date-time 2017 12 18))
           "= December 18, 2017, Monday - <SUMMARY GOES HERE> =" )))

  (testing "test get-filename-extension-str returns correct format"
    (is (= (get-filename-extension-str (t/date-time 2017 3 18))  "2017-03-18.wiki"))
    (is (= (get-filename-extension-str (t/date-time 2014 2 8))   "2014-02-08.wiki"))
    (is (= (get-filename-extension-str (t/date-time 2027 1 1))   "2027-01-01.wiki"))
    (is (= (get-filename-extension-str (t/date-time 2017 12 14)) "2017-12-14.wiki")))
